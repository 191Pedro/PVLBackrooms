...
const optionOperation = html.get('#filter-per-operation');
optionOperation.addEventListener('change', calculateFilters);
const optionSector = html.get('#filter-per-sector');
optionSector.addEventListener('change', calculateFilters);
...
function calculateFilters() {
    const displayingOperation = html.get('#displaying-operation');
    displayingOperation.textContent = optionOperation.selectedOptions[0].textContent;
    const displayingSector = html.get('#displaying-sector');
    displayingSector.textContent = optionSector.selectedOptions[0].textContent;
    const displayingLevel = html.get('#displaying-level');
    displayingLevel.textContent = optionLevel.selectedOptions[0].textContent;
    
    const selectedOperation = optionOperation.value;
    const selectedSector = optionSector.value;
    const selectedLevel = optionLevel.value;
    const searchValue = formatString(searchInput.value);

    dataFiltered = data
        .filter(item => (item.dataset.operation == selectedOperation) || (selectedOperation == 'all'))
        .filter(item => (item.dataset.sector.includes(selectedSector)) || (selectedSector == 'all'))
